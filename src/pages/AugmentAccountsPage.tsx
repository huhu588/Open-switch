...
export function AugmentAccountsPage() {
  ...
  const augmentProviders = useMemo(
    () => store.providers.filter((p) => p.model_type === 'augment'),
    [store.providers],
  );

  useEffect(() => {
    store.loadProviders();
    invoke<boolean>('detect_augment_extension')
      .then((found) => { setConfigStatus(found ? 'detected' : 'not-found'); })
  ...
  const handleRefresh = async () => {
    ...
    try {
      const found = await invoke<boolean>('detect_augment_extension');
      setConfigStatus(found ? 'detected' : 'not-found');
    } catch { setConfigStatus('not-found'); }
  };

  return (
    <div className="page-container">
      <PlatformOverviewTabsHeader platform="augment" active={activeTab} onTabChange={setActiveTab} />
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          <div className="card" style={{ padding: 20 }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Puzzle size={18} />
              Augment {t('platformOverview.status', '状态')}
            </h3>
              ...
              {configStatus === 'detected' && (
                <>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-primary)' }}>
                    Augment {t('platformOverview.extensionDetected', '扩展已检测到')}
                  </span>
                </>
              )}
              {configStatus === 'not-found' && (
                <>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>
                    Augment {t('platformOverview.extensionNotDetected', '扩展未检测到')}
                  </span>
                </>
              )}
            </div>
          ...
          {augmentProviders.length > 0 && (
            <div className="card" style={{ padding: 20, marginTop: 16 }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: 0 }}>
                {t('platformOverview.providerCount', '已配置 {{count}} 个 Provider', { count: augmentProviders.length })}
              </p>
            </div>
          )}
        </div>
