...
const DEFAULT_REPO = 'jlcodes99/ai-switch';
const DEFAULT_CASK_PATH = 'Casks/ai-switch.rb';
const DEFAULT_TARGET = 'universal-apple-darwin';
const DEFAULT_RELEASE_ASSET_PREFIX = 'AI.Switch';
...
function printHelp() {
  ...
  console.log('  --repo <owner/repo>     GitHub repo for release (default: jlcodes99/ai-switch)');
  console.log('  --cask <path>           Homebrew cask file path (default: Casks/ai-switch.rb)');
...
function resolveSourceDmgPath(version, options) {
  ...
  const defaultPath = path.resolve(
    ...
    'bundle',
    'dmg',
    `AI Switch_${version}_universal.dmg`
  );
...
function main() {
  ...
  console.log('AI Switch GitHub Release + Homebrew cask publisher');
  ...
  console.log('- 检查 Casks/ai-switch.rb diff');
