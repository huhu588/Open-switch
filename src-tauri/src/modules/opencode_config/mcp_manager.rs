// MCP 配置管理器
// 负责管理 ~/.Ai Switch/mcp/ 目录下的多个 JSON 文件，并同步到 opencode.json

use crate::modules::opencode_config::models::{McpConfig, McpOAuthConfig, McpServer, McpServerType};
...
/// MCP 配置管理器
/// 采用目录模式：~/.Ai Switch/mcp/ 下每个服务器一个 JSON 文件
pub struct McpConfigManager {
    mcp_dir: PathBuf,       // ~/.Ai Switch/mcp/
...
impl McpConfigManager {
    ...
    pub fn sync_to_opencode(&self, server_names: Option<&[String]>) -> Result<(), String> {
        ...
        if let Some(obj) = opencode_data.as_object_mut() {
            obj.remove("$schema");
        }
